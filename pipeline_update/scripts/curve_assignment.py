...
def _assign_one(manu: str, model: str, rated_kW: float,
    ...
    from oedb_catalog import normalize, MANUFACTURER_ALIASES, strip_mfr_prefix
    ...
    # Tier 1: curated OEM curves (manufacturer datasheets).
    # Runs ahead of oedb because oedb is European-curated and missing
    # most popular US fleet models — having it run first lets oedb's
    # fuzzy-by-geometry fallback grab a wrong-but-close European model
    # for our GE 2.x and SWT 2.3 turbines, which is worse than the
    # oem-curated curve.
    from oem_curves import lookup_oem_curve
    mod_n_stripped = strip_mfr_prefix(mfr_n, mod_n)
    for try_mod in (mod_n, mod_n_stripped):
        if not try_mod:
            continue
        oem = lookup_oem_curve(mfr_n, try_mod)
        if oem is not None:
            # Use USWTDB rotor diameter if available — more reliable than
            # the brochure value for repowered/uprated machines.
            if np.isfinite(rotor_d) and rotor_d > 0:
                oem = PowerCurve(
                    name=oem.name, rated_kW=oem.rated_kW,
                    rotor_diameter_m=float(rotor_d),
                    wind_speeds_m_s=oem.wind_speeds_m_s,
                    power_kW=oem.power_kW,
                    cut_in=oem.cut_in, cut_out=oem.cut_out,
                    iec_class=oem.iec_class, source=oem.source,
                )
            return oem, oem.source

    # Tier 2: oedb catalog
    if oedb_catalog is not None:
        cat = oedb_catalog.lookup(
            manu, model,
            uswtdb_rotor_d=rotor_d if np.isfinite(rotor_d) else None,
            uswtdb_rated_kW=rated_kW if np.isfinite(rated_kW) else None,
        )
        if cat is not None:
    ...
    # Tier 3: NREL generic by specific power
