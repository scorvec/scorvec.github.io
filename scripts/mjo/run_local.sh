...
( "$PY" src/walker.py --date "$DATE" --time "$TIME" \
  ...
  # Hadley intensity + poleward-edge series vs the ERA5 metric climatology
  # (hadley_clim.nc; rebuild only when the ERA5 sample cache changes).
  "$PY" src/hadley_timeseries.py \
...
perl -0pi -e "s/((?:aam|aam_trend|aam_phase|mmsf_anom|walker_anom|jets|torque_timeseries|torque_ranges|hadley_timeseries)\.webp)\?v=\w+/\${1}?v=$CB/g" "$REPO/enso-atmosphere.html" 2>/dev/null || true
