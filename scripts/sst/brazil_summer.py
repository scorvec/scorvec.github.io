...
def retrieve(years, month, dest: Path) -> bool:
    ...
    req = {"originating_centre": CENTRE, "system": SYSTEM,
           ...
           "leadtime_month": ["4", "5", "6"],      # Aug init -> Dec, Jan, Feb
...
def era5_djf():
    ...
    for d, x in daily.items():
        ...
        if m == 12:
            ...
            out.setdefault(y - 1, []).append(x)
    return {y: float(np.mean(v)) for y, v in out.items() if len(v) > 80}
...
def main() -> int:
    ...
    print(f"ERA5 observed DJF pop-weighted: {yrs[0]}/{str(yrs[0]+1)[2:]}.."
    ...
    n = min(len(v) for v in per_month.values())
    djf = np.mean([per_month[m][:n] for m in (12, 1, 2)], axis=0)
    ...
    out = {"generated": now.strftime("%Y-%m-%d %H:%M UTC"),
           "target": "DJF 2m temperature over Brazil, metro-population weighted",
           "metros": len(METROS),
           ...
           "forecast_p10_p50_p90": [round(float(np.percentile(djf, p)), 2)
                                    for p in (10, 50, 90)],
           "p_warmest_on_record": round(p_record, 3),
           "p_top3": round(float(np.mean(djf > sorted(
...
def figure(out, obs, yrs, djf, record):
    ...
    hd.text(0.015, 0.20, f"DJF 2-m temperature, weighted by metro population "
