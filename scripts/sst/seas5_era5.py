...
PULLS = {
    ...
    "am_e": dict(dataset="reanalysis-era5-single-levels-monthly-means",
                 req=dict(product_type=["monthly_averaged_reanalysis"], variable=["evaporation"],
                          year=YEARS, month=MONTHS, time=["00:00"], area=[75, -170, -60, -30], grid=[1.0, 1.0], data_format="grib")),
    # global 1° references for the single-map normals (t2m, tp; z500) — the local store fills in until these land
    "gl_t": dict(dataset="reanalysis-era5-single-levels-monthly-means",
                 req=dict(product_type=["monthly_averaged_reanalysis"], variable=["2m_temperature", "total_precipitation"],
                          year=[str(y) for y in range(1991, 2026)], month=[f"{m:02d}" for m in range(1, 13)], time=["00:00"],
                          area=[90, -180, -90, 180], grid=[1.0, 1.0], data_format="grib")),
    "gl_z": dict(dataset="reanalysis-era5-pressure-levels-monthly-means",
